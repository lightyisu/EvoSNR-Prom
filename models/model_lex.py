...
class HyenaSegment(nn.Module):
 ...
 #维度定义
        ...
        # ===== 改进核心：门控融合模块 =====
        # 替代原来的 torch.cat 暴力拼接
        self.gated_fusion = GatedFusion(
            evo_dim=self.evo_dim,
            kmer_dim=self.kmer_input_dim
        )

        # NERmodel 的 input_dim 从 812 改为 512
        # 因为门控融合输出就是 512 维，不再需要 812 = 512 + 300
        self.NERmodel = NERmodel(model_type='transformer', input_dim=self.evo_dim, hidden_dim=self.evo_dim, num_layer=4, biflag=True)
    ...
    def forward(self, input_seqs=None, label_ids=None,kmer_embeddings=None,attention_mask=None):
        ...
        embed, _ = self.evo(input_ids) # (batch, length, evo_dim=512)
        fused_embeddings, gate_values = self.gated_fusion(embed, kmer_embeddings)
        fused_embeddings = self.dropout(fused_embeddings)  # 只做一次 dropout
        # (batch, seq_len, 512) → Transformer NER
        ...
        outputs = (logits,) + (fused_embeddings,)
...
class GatedFusion(nn.Module):
    ...
    def __init__(self, evo_dim, kmer_dim):
        super().__init__()
