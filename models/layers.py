...
class AttentionModel(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # x: embedding (b, l, we)
        if x.dim() != 3:
            raise ValueError(f"AttentionModel expects a 3D tensor of shape (batch, seq_len, hidden), got {tuple(x.shape)}")
...
class NERmodel(nn.Module):
    ...
    def __init__(self, model_type, input_dim, hidden_dim, num_layer, dropout=0.2, gpu=True, biflag=True):
    ...
    def forward(self, input, mask=None):
        ...
        if self.model_type == 'transformer':
            self.attention_weights = []  # 每次 forward 重置
            feature_out_d = self.attention_model(input, mask)
    # 捕获每层的注意力权重
            for layer in self.attention_model.layers:
